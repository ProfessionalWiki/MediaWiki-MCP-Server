...
| `MCP_USER_AGENT` | Replaces the `User-Agent` header of every outbound request, so wiki operators can reach you rather than this project. Give a name and an email or URL, e.g. `ExampleBot/1.0 (bot@example.org)`: Wikimedia wikis [limit unauthenticated requests](https://www.mediawiki.org/wiki/Wikimedia_APIs/Rate_limits) from a `User-Agent` without a contact to 10 a minute, across all of them. | `mediawiki-mcp-server/<version> (https://github.com/ProfessionalWiki/MediaWiki-MCP-Server)` |
