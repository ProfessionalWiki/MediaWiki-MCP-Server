...
| `MCP_OAUTH_CIMD_ALLOWED_HOSTS` | Extra hosts to trust for clients that identify by a vendor-hosted URL (Client ID Metadata Documents): comma-separated bare hosts or `host:port`. VS Code, ChatGPT, and Zed are trusted by default. See [docs/deployment.md — allowing more clients](docs/deployment.md#allowing-more-clients). | `unset` |
