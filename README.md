...
- **Per-request bot password (HTTP).** Each request carries `Authorization: Basic <base64 of "username:password">` and the server acts as that user, so no credentials live on the server. On by default over HTTP; run it behind TLS. See [docs/deployment.md — per-request bot password](docs/deployment.md#per-request-bot-password-http-transport).
