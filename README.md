...
| `MCP_OAUTH_CIMD_ALLOWED_HOSTS` | Additional client_id document hosts the hosted proxy will fetch and trust for Client ID Metadata Documents (CIMD): comma-separated bare hosts or `host:port`. The verified first-party hosts (vscode.dev, claude.ai, zed.dev, chatgpt.com) are always trusted. See [docs/deployment.md — Client ID Metadata Documents](docs/deployment.md#client-id-metadata-documents). | `unset` |
