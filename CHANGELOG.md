...
- `delete-page` no longer sends an empty deletion reason on a wiki that sets `attributeEdits: false` and a call that gave no comment. MediaWiki recorded that empty reason verbatim, leaving a blank deletion log entry; with no reason sent at all it can autogenerate its own `content was: …` reason instead.
