...
- `smw-ask` tool — runs a Semantic MediaWiki `#ask` query against the active wiki. Returns row-shaped results with one entry per matching subject. Enabled only when the wiki has Semantic MediaWiki installed (auto-detected via `siteinfo&siprop=extensions`).
- `smw-list-properties` tool — lists SMW properties on the active wiki with usage templates ready to paste into `smw-ask`. Same gating as `smw-ask`.
...
[Unreleased]: https://github.com/ProfessionalWiki/MediaWiki-MCP-Server/compare/v0.8.0...HEAD
