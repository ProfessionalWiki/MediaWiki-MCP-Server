...
- Hosted OAuth proxy for the HTTP transport: the server can act as an OAuth 2.1 Authorization Server in front of a single pre-registered MediaWiki consumer. An OAuth-aware MCP client pointed at `https://<wiki>/mcp` discovers the server, registers itself, and signs each user in as themselves — no manual token handling — while the server brokers the upstream MediaWiki auth-code+PKCE flow and mints its own audience-bound access/refresh JWTs. Anonymous read is preserved; write tools step up to a `401` challenge only when invoked. Enabled when the HTTP transport runs with `MCP_PUBLIC_URL` and `MCP_OAUTH_JWT_SIGNING_KEY` set and the default wiki has an `oauth2ClientId`. See [docs/deployment.md — Shape 3](docs/deployment.md#shape-3--single-wiki-hosted-server-mediated-oauth-proxy).
- When the hosted OAuth proxy is enabled, the HTTP transport serves RFC 8414 authorization-server metadata (`/.well-known/oauth-authorization-server`, plus the `…/mcp` variant), an RFC 7591 Dynamic Client Registration endpoint, an `/authorize` endpoint with a browser consent page, a fixed `/oauth/callback`, and an RFC 6749 `/token` endpoint. The root `/.well-known/oauth-protected-resource` (RFC 9728) advertises the proxy itself as the authorization server.
- `publicServer` per-wiki config field: the wiki's browser-facing public base URL, used when it differs from the internal `server` (e.g. a Docker-network alias). In the hosted OAuth proxy it is the host the user's browser is redirected to for the upstream consent screen, while `server` carries the internal API traffic and the server→wiki token exchange. Falls back to `server` when unset.
- `MCP_PUBLIC_URL`, `MCP_OAUTH_JWT_SIGNING_KEY`, `MCP_OAUTH_TOKEN_TTL` (default `55m`), and `MCP_OAUTH_CONSENT_TTL` (default `30d`) environment variables configure the hosted OAuth proxy. `MCP_PUBLIC_URL` is the proxy's public issuer/base; the signing key (≥32 chars, kept fixed so issued tokens survive a restart) signs the proxy's JWTs and consent cookies; the two TTLs accept a `55m`/`1h`/`30d`-style duration or bare seconds.
