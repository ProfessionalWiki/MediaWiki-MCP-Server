...
- `delete-page` no longer leaves a blank deletion log entry on a wiki that sets `attributeEdits: false`. With no caller comment it sent an empty reason, which suppressed the `content was: …` reason MediaWiki writes when it is given none; it now sends no reason, so MediaWiki writes its own.
