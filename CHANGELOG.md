...
- A host correcting its clock no longer changes what the server does. Every elapsed time — a rate-limit allowance, the shutdown grace window, the readiness and extension-detection caches, a sign-in's lifetime — was measured against the wall clock, which steps in both directions on an NTP correction or a virtualised host resuming. A backwards step could refuse a caller that had barely touched its rate-limit allowance, with a `Retry-After` of up to an hour; a forwards step could end a graceful shutdown early, aborting the tool calls it was waiting for and reporting a clean shutdown as a crash.
