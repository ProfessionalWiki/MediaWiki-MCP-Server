...
describe( 'ResponseFormatterImpl', () => {
	...
	it( 'notFound is a typed shorthand for error("not_found", ...)', () => {
		expect( fmt.notFound( 'X', 'missingtitle' ) ).toEqual(
			fmt.error( 'not_found', 'X', 'missingtitle' )
		);
	} );
	...
	it( 'conflict is a typed shorthand for error("conflict", ...)', () => {
		expect( fmt.conflict( 'X', 'editconflict' ) ).toEqual(
			fmt.error( 'conflict', 'X', 'editconflict' )
		);
	} );
